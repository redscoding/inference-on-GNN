...
#from tensorflow.examples.tutorials.mnist.input_data as input_data
...
dtype = tf.float32
conv_initializer = tf.compat.v1.initializers.glorot_uniform()  # tf.initializers.truncated_normal(dtype=dtype, mean=0, stddev=0.1)
...
input_layer = tf.compat.v1.placeholder(dtype=dtype, shape=(None, input_dimensions, input_dimensions, 1), name='input')  # [0, 1]
expected_output = tf.compat.v1.placeholder(dtype=tf.int32, shape=(None, input_dimensions, input_dimensions),
                                 name='expected_output')  # [0, 1]

hyper_h = tf.compat.v1.get_variable(tf.compat.v1.random.truncated_normal(dtype=dtype, shape=(1,), mean=0, stddev=1), name='hyper_h')  #
hyper_eta = tf.compat.v1.get_variable(
    tf.compat.v1.random.truncated_normal(dtype=dtype, shape=(1, input_dimensions, input_dimensions, 1), mean=0, stddev=1),
    name='hyper_eta')
hyper_beta = tf.compat.v1.get_variable(
    tf.compat.v1.random.truncated_normal(dtype=dtype, shape=(1, input_dimensions_2, input_dimensions_m1, 1), mean=0, stddev=1),
    name='hyper_beta')
...
class GNN_GRU:
    def __init__(self):
        ...
        self.CM_2 = Conv2D(filters=message_hidden_size, kernel_size=1, activation=tf.nn.relu,
                           kernel_initializer=conv_initializer, name='CM_2')
        self.CM_1 = Conv2D(filters=message_hidden_size, kernel_size=1, activation=tf.nn.relu,
                           kernel_initializer=conv_initializer, name='CM_1')
        self.CM_0 = Conv2D(filters=message_size * 2, kernel_size=1, activation=tf.nn.relu,
                           kernel_initializer=conv_initializer, name='CM_0')  # * 2

        # Weights for input vectors of shape (message_size, hidden_size)
        self.Wr = Conv2D(filters=hidden_size, kernel_size=1, activation=None, kernel_initializer=conv_initializer,
                         use_bias=True, name='C_Wr')
        self.Wz = Conv2D(filters=hidden_size, kernel_size=1, activation=None, kernel_initializer=conv_initializer,
                         use_bias=True, name='C_Wz')
        self.Wh = Conv2D(filters=hidden_size, kernel_size=1, activation=None, kernel_initializer=conv_initializer,
                         use_bias=True, name='C_Wh')

        # Weights for hidden vectors of shape (hidden_size, hidden_size)
        self.Ur = Conv2D(filters=hidden_size, kernel_size=1, activation=None, kernel_initializer=conv_initializer,
                         use_bias=True, name='C_Ur')
        self.Uz = Conv2D(filters=hidden_size, kernel_size=1, activation=None, kernel_initializer=conv_initializer,
                         use_bias=True, name='C_Uz')
        self.Uh = Conv2D(filters=hidden_size, kernel_size=1, activation=None, kernel_initializer=conv_initializer,
                         use_bias=True, name='C_Uh')

        # Biases for hidden vectors of shape (hidden_size,)
        # self.br = tf.compat.v1.get_variable(tf.compat.v1.random.truncated_normal(dtype=dtype, shape=(1, 1, hidden_size), mean=0, stddev=0.01), name='br')
        # self.bz = tf.compat.v1.get_variable(tf.compat.v1.random.truncated_normal(dtype=dtype, shape=(1, 1, hidden_size), mean=0, stddev=0.01), name='bz')
        # self.bh = tf.compat.v1.get_variable(tf.compat.v1.random.truncated_normal(dtype=dtype, shape=(1, 1, hidden_size), mean=0, stddev=0.01), name='bh')
        ...
        self.H_cur = tf.squeeze(tf.slice(self.H_ts, [time_step - 1, 0, 0, 0, 0], [1, -1, -1, -1, -1]), axis=0,
                                name='H_cur')
...
map_2 = Conv2D(filters=message_hidden_size, kernel_size=1, activation=tf.nn.relu, kernel_initializer=conv_initializer,
               name='CR_2')(H_cur)
map_1 = Conv2D(filters=message_hidden_size, kernel_size=1, activation=tf.nn.relu, kernel_initializer=conv_initializer,
               name='CR_1')(map_2)
map_0 = Conv2D(filters=label_size, kernel_size=1, activation=tf.nn.softmax, kernel_initializer=conv_initializer,
               name='CR_0')(map_1)
...
repar_binary = tf.multiply(tf.expand_dims(binary_edge, axis=-1), tf.constant(ones_repar_binary, dtype=dtype),
                           name='repar_binary')
repar_degree = main_layers.edge2node(tf.ones([1, input_dimensions_2, input_dimensions_m1, 2]), size=1,
                                     name='repar_degree')  # 2
...
loss = tf.reduce_mean(- (one_hot_output + tf.stop_gradient(bp_lambda * one_hot_V_t)) * tf.log(output_layer),
                      name='loss') + 0.001 * tf.square(hyper_h)  #
...
for epoch in range(num_epoch):
    ...
    X_nn, X_bp = session.run([label_layer, V_label],
                             feed_dict={input_layer: X_test[:batch_size], expected_output: Y_test[:batch_size]})  #
